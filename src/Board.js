 ...
 */
    ...
    hasMajorDiagonalConflictAt: function (col) {
      let temp = [];
      let count = 0;
      let attrNum = this.attributes.n;
      if (Math.abs(col) >= attrNum - 1) {
        return false;
      } else if (col >= 0) {
        for (let i = 0; i < attrNum - col; i++) {
    ...
    hasMinorDiagonalConflictAt: function (col) {
      ...
      if (col === 0 || col >= ((len) * 2)) {
        ...
        for (let i = 0; i < len * 2 - col + 1; i++) {
          temp.push(this.get(Math.abs(len - col) + i)[len - i]);
        }
      }
      for (let i = 0; i < temp.length; i++) {
        if (temp[i] !== 0) {
